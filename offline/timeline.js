modelfilestrs['timeline'] = hereDoc(function(){/*!
				...
				.click(function() { // mark labels and show feedback
					...
					if (empty){
						...
						if (correct == 0) {
							...
							if(XTGetMode() == "normal")
								$feedback.html(x_currentPageXML.getAttribute("scoreTracking") != undefined ? x_currentPageXML.getAttribute("scoreTracking").replace("{i}", correct).replace("{n}", $("#labelHolder .label").length) : x_addLineBreaks(x_currentPageXML.getAttribute("feedback")));
							else	
